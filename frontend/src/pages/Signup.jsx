  ...
  const handleGoogleSignup = async (profile) => {
    ...
    try {
      ...
      const savedUser = {
        ...
        profile_photo_path:
          result.user?.profile_photo_path || profile.picture || "",
      };
  ...
  const handleUsernameCheck = async () => {
    ...
    try {
      ...
      setUsernameCheck({
        ...
        message:
          result.available ? "사용 가능한 아이디입니다." : (
            "이미 사용 중인 아이디입니다."
          ),
      });
    ...
    <AuthVideoLayout className="page-enter font-cute">
      <div className="auth-signup-shell flex w-full max-w-[430px] flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-5 pt-4 pb-6 sm:px-8">
          <div className="auth-glass-card rounded-3xl p-6 shadow-lg">
        ...
        >
          {err && (
            <p
              className="mb-2.5 text-center text-sm font-bold"
