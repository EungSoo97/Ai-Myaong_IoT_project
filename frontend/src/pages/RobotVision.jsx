  ...
  return (
    <div className="px-5 pt-5 pb-6">
      ...
      <Card className="overflow-hidden" data-tour="vision-stream">
        <div ref={fsRef} className={isFullscreen ? 'fullscreen-stage' : 'relative w-full aspect-video bg-gradient-to-br from-brand-cream to-brand-line dark:from-[#2b2520] dark:to-black overflow-hidden'}>
          {isFullscreen ? (
            ...
            <>
              ...
              <div className="absolute top-3 left-3 flex flex-wrap items-center gap-2">
                <span className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-white text-[11px] font-bold ${camStatus === 'live' ? 'bg-black/55' : 'bg-black/40'}`}>
                  <span className={`w-2 h-2 rounded-full ${camStatus === 'live' ? 'bg-red-400 animate-pulse' : camStatus === 'connecting' ? 'bg-amber-300 animate-pulse' : 'bg-white/40'}`} />
                  {camStatus === 'live' ? 'LIVE' : camStatus === 'connecting' ? '연결 중' : '오프라인'}
                </span>
                {recording && (
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center" onClick={dismiss}>
      ...
      >
        ...
        {/* 영상 */}
        <div className="mt-4 relative aspect-video rounded-2xl overflow-hidden bg-gradient-to-br from-[#2b2520] to-black">
          {hasMedia ? (
            <>
              {showImage ? (
                <img
...
function StreamFrame({ src, mode, error, className = '', fullscreen = false, onStatusChange }) {
  ...
  const status = error || imageError ? 'off' : src && loaded ? 'live' : 'connecting'
  ...
  return (
    <div className={`${className} bg-brand-cream dark:bg-black flex items-center justify-center overflow-hidden`}>
      {src && !imageError && (
        <>
          <img
            src={src}
