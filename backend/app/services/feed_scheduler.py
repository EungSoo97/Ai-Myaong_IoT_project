...
def _has_feed_log_this_minute(db, user_id, pet_id, amount, minute_start, minute_end):
    return (
...
def _has_water_log_this_minute(db, user_id, pet_id, amount, minute_start, minute_end):
    return (
        ...
        .filter(
            ...
            WaterLog.water_amount_ml == amount,
...
def _run_once(feed_service, hhmm):
    ...
    try:
        ...
        handled_feed = set()
        handled_water = set()
        for s in db.query(Settings).with_for_update().all():
            ...
            for item in _parse(s.feed_schedule):
                if item.get("on", True) and str(item.get("time")) == hhmm:
                    ...
                    if not pet_id:
                        continue
                    feed_key = (s.user_id, pet_id, amount, minute_start)
                    if feed_key in handled_feed:
                        continue
                    if _has_feed_log_this_minute(db, s.user_id, pet_id, amount, minute_start, minute_end):
                        continue
            ...
            for item in _parse(s.water_schedule):
                if item.get("on", True) and str(item.get("time")) == hhmm:
                    ...
                    if not pet_id:
                        continue
                    water_key = (s.user_id, pet_id, amount, minute_start)
                    if water_key in handled_water:
                        continue
                    if _has_water_log_this_minute(db, s.user_id, pet_id, amount, minute_start, minute_end):
                        continue
